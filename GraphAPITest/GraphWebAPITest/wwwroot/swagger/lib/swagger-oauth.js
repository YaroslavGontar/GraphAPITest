...
function handleLogin() {
    var e = [],
        o = window.swaggerUi.api.authSchemes || window.swaggerUi.api.securityDefinitions;
    if (o) {
        var i,
            n = o;
        for (i in n) {
                ...
                if (i.hasOwnProperty(h) && l.indexOf(h) != -1) {
                    ...
                    if ("oauth2" !== i[h].type || !g || "implicit" !== g && "accessCode" !== g) {
                        if ("oauth2" === i[h].type && g && "application" === g) {
                            var w = i[h];
                            return window.swaggerUi.tokenName = 'id_token',
                        ...
                        if (i[h].grantTypes) {
                            ...
                            for (var f in c)
                                if (c.hasOwnProperty(f) && "implicit" === f) {
                                    var w = c[f];
                                    w.loginEndpoint.url;
                                    r = w.loginEndpoint.url + "?response_type=id_token",
                                        window.swaggerUi.tokenName = 'id_token'
                        ...
                        var w = i[h];
                        r = w.authorizationUrl + "?response_type=" + ("implicit" === g ? "id_token" : "code"),
                            window.swaggerUi.tokenName = "id_token",
                            window.swaggerUi.tokenUrl = "accessCode" === g ? w.tokenUrl : null,
                            o = h
                    }
                }
            redirect_uri = p,
...
function handleLogout() {
    ...
    window.enabledScopes = null,
        $(".authorize__btn").addClass("ic-off"),
        $(".authorize__btn").removeClass("ic-on"),
...
function initOAuth(e) {
    ...
    return appName = o.appName || i.push("missing appName"),
        ...
        i.length > 0 ? void log("auth unable initialize oauth: " + i) : ($("pre code").each(function (e, o) {
            hljs.highlightBlock(o)
    }), $(".authorize__btn").unbind(), void $(".authorize__btn").click(function (e) {
...
window.processOAuthCode = function (e) {
    ...
    if (e)
        if (e.error) {
            ...
            if (o || (o = e.state), n) {
                var a = null;
                $(".authorize__btn").addClass("ic-on");
                $(".authorize__btn").removeClass("ic-off");
